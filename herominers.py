...
def herominers_requests():
    ...
    herominers_output = f"""
    <a>Hashrates:<br><br>
    {rig_1}:    {rig_1_current_hashrate}<br>
    {rig_2}:    {rig_2_current_hashrate}<br>
    Combined now:   {current_hashrate}<br>
    Combined 24H:   {avg_24h_hashrate}<br><br>
    Ergo:<br><br>
    Total paid: {total_paid_ergo}<br>
    Pending balance: {pending_balance_ergo}<br>
    """

    #Printing out the data in correct form
    with open("/var/www/html/herominers_output.html", "w") as file:
        file.write(f"{herominers_output}")
